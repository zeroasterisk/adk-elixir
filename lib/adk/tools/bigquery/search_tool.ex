defmodule ADK.Tool.BigQuery.SearchTool do
  ...
  defp do_search(prompt, project_id, credentials, settings, opts) do
    ...
    projects_to_filter = if project_ids_filter && length(project_ids_filter) > 0, do: project_ids_filter, else: [project_id]
    
    query_parts = if length(projects_to_filter) > 0 do
      [construct_search_query_helper("projectid", "=", projects_to_filter) | query_parts]
    else
      query_parts
    end

    query_parts = if dataset_ids_filter && length(dataset_ids_filter) > 0 do
      dataset_resource_filters = for pid <- projects_to_filter, did <- dataset_ids_filter do
    ...
    query_parts = if types_filter && length(types_filter) > 0 do
    ...
    if Keyword.has_key?(opts, :mock_error) do
      ...
      processed_results = Enum.map(results, fn r ->
        %{
          "name" => Map.get(r, "name", ""),

defmodule ADK.Tool.OpenApiTool.Auth.AuthHelpers do
  ...
  def openid_dict_to_scheme_credential(config_dict, scopes, credential_dict) do
    ...
    if missing_config != [] do
      raise ArgumentError, "Invalid OpenID Connect configuration: missing #{Enum.join(missing_config, ", ")}"
    end
    ...
    credential_dict = if map_size(credential_dict) == 1 do
      {_key, value} = Enum.at(credential_dict, 0)
      if is_map(value) and Map.has_key?(value, "client_id") and Map.has_key?(value, "client_secret") do
        value
      else
        credential_dict
      end
    ...
    if missing_cred != [] do
      raise ArgumentError, "Missing required fields in credential_dict: #{Enum.join(missing_cred, ", ")}"
    end

    cred = Credential.open_id_connect(
      nil,
      client_id: credential_dict["client_id"],
      client_secret: credential_dict["client_secret"]
    )
    cred = if Map.has_key?(credential_dict, "redirect_uri") do
      %{cred | metadata: Map.put(cred.metadata, "redirect_uri", credential_dict["redirect_uri"])}
    else
      cred
    end
  ...
  def openid_url_to_scheme_credential(openid_url, scopes, credential_dict) do
    case Req.get(openid_url, receive_timeout: 10_000, retry: false) do
      {:ok, %{status: status, body: body}} when status in 200..299 ->
        config_dict = case body do
          b when is_map(b) -> b
          b when is_binary(b) ->
            case Jason.decode(b) do
              {:ok, decoded} -> decoded
              {:error, _} -> raise ArgumentError, "Invalid JSON response from OpenID configuration endpoint #{openid_url}: Invalid JSON"
            end
        end
      ...
      {:ok, %{status: status}} ->
        raise ArgumentError, "Failed to fetch OpenID configuration from #{openid_url}: HTTP #{status}"
      {:error, %Jason.DecodeError{} = err} ->
        raise ArgumentError, "Invalid JSON response from OpenID configuration endpoint #{openid_url}: #{inspect(err)}"
      {:error, reason} ->
        raise ArgumentError, "Failed to fetch OpenID configuration from #{openid_url}: #{inspect(reason)}"
    end
  end
  ...
  def credential_to_param(%{"type" => "apiKey"} = auth_scheme, %Credential{type: :api_key, api_key: api_key} = _cred) when not is_nil(api_key) do
  ...
  def credential_to_param(%{"type" => "http"}, %Credential{type: :http_bearer, metadata: %{"basic" => true}}) do
    raise ArgumentError, "Basic Authentication is not supported."
  end

  def credential_to_param(%{"type" => "http"} = auth_scheme, %Credential{type: :http_bearer, access_token: token} = _cred) when not is_nil(token) do
    param = %{
  ...
  def credential_to_param(%{"type" => "oauth2"} = auth_scheme, %Credential{type: :http_bearer, access_token: token} = _cred) when not is_nil(token) do
    param = %{
      original_name: "Authorization",
      param_location: "header",
      py_name: @internal_auth_prefix <> "Authorization",
  ...
  def credential_to_param(%{"type" => "openIdConnect"} = auth_scheme, %Credential{type: :http_bearer, access_token: token} = _cred) when not is_nil(token) do
    param = %{
      original_name: "Authorization",
      param_location: "header",
      py_name: @internal_auth_prefix <> "Authorization",
  ...
  def dict_to_auth_scheme(data) do
    ...
    case security_type do
      ...
      "oauth2" -> data
      "openIdConnect" -> data
      _ -> raise ArgumentError, "Invalid security scheme type: #{security_type}"
    end
  end
end
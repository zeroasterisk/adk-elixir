defmodule ADK.StreamingParityTest do
  ...
  describe "Streaming parity" do
    test "test_streaming (basic turn complete)" do
      ...
      pid = self()
      res_events = Runner.run_streaming(runner, "user-1", "sess-1", "Hello",
    ...
    test "test_live_streaming_function_call_single" do
      ADK.LLM.Mock.set_responses([
        %{
          function_call: %{name: "get_weather", args: %{"location" => "San Francisco", "unit" => "celsius"}}
        },
        "turn complete"
      ])

      tool = FunctionTool.new("get_weather",
      ...
      pid = self()
      res_events = Runner.run_streaming(runner, "user-1", "sess-2", "What is the weather in San Francisco?",
        on_event: fn event -> send(pid, {:streamed, event}) end
      )

      assert length(res_events) >= 1

      function_call_found = Enum.any?(res_events, fn event ->
        Event.has_function_calls?(event) and hd(Event.function_calls(event)).name == "get_weather"
      end)
      assert function_call_found
      
      fc_part = Enum.find_value(res_events, fn e -> 
      ...
      function_response_found = Enum.any?(res_events, fn event ->
        Event.has_function_responses?(event) and hd(Event.function_responses(event)).name == "get_weather"
      end)
      assert function_response_found

      fr_part = Enum.find_value(res_events, fn e -> 
    ...
    test "test_live_streaming_function_call_multiple" do
      ...
      weather_tool = FunctionTool.new("get_weather", description: "Get weather", func: fn _ctx, %{"location" => loc} ->
        %{"temperature" => 22, "condition" => "sunny", "location" => loc}
      end)
      
      time_tool = FunctionTool.new("get_time", description: "Get time", func: fn _ctx, %{"timezone" => tz} ->
      ...
      pid = self()
      res_events = Runner.run_streaming(runner, "user-1", "sess-3", "What is the weather and current time?",
        on_event: fn event -> send(pid, {:streamed, event}) end
      )

      assert length(res_events) >= 1

      weather_call_found = Enum.any?(res_events, fn event ->
        Event.has_function_calls?(event) and hd(Event.function_calls(event)).name == "get_weather"
      end)
      time_call_found = Enum.any?(res_events, fn event ->
    ...
    test "test_live_streaming_function_call_parallel" do
      ...
      pid = self()
      res_events = Runner.run_streaming(runner, "user-1", "sess-4", "Compare weather in SF and NYC",
        on_event: fn event -> send(pid, {:streamed, event}) end
      )

      assert length(res_events) >= 1

      sf_call_found = Enum.any?(res_events, fn event ->
        Enum.any?(Event.function_calls(event), fn fc ->
          fc.name == "get_weather" and fc.args["location"] == "San Francisco"
        end)
      end)

      nyc_call_found = Enum.any?(res_events, fn event ->
        Enum.any?(Event.function_calls(event), fn fc ->
    ...
    test "test_live_streaming_function_call_with_error" do
      ...
      pid = self()
      res_events = Runner.run_streaming(runner, "user-1", "sess-5", "What is weather in Invalid Location?",
        on_event: fn event -> send(pid, {:streamed, event}) end
      )
    ...
    test "test_live_streaming_function_call_sync_tool" do
      ...
      calc_tool = FunctionTool.new("calculate", description: "Calculate", func: fn _ctx, %{"x" => x, "y" => y} ->
      ...
      pid = self()
      res_events = Runner.run_streaming(runner, "user-1", "sess-6", "Calculate 5 plus 3",
        on_event: fn event -> send(pid, {:streamed, event}) end
      )
    ...
    test "test_live_streaming_text_content_persisted_in_session" do
      ...
      runner = %Runner{app_name: "test", agent: agent, session_store: {ADK.Session.Store.InMemory, []}}

      {:ok, session_pid} = ADK.Session.start_supervised(
        app_name: "test",
        user_id: "user-1",
        session_id: "sess-7",
      ...
      user_content_found = Enum.any?(events, fn e -> 

defmodule ADK.Workflow.SagaCompensationTest do
  ...
  test "saga executes steps sequentially when successful", %{ctx: ctx} do
    ...
    step1 = Workflow.run(:step1, fn _ctx ->
    ...
    step2 = Workflow.run(:step2, fn _ctx ->
  ...
  test "saga rolls back in reverse order on failure", %{ctx: ctx} do
    ...
    step3 = Workflow.run(:step3, fn _ctx ->
    ...
    assert Enum.any?(events, fn e -> e.author == "workflow" and String.contains?(ADK.Event.text(e), "simulated_failure") end)
  end

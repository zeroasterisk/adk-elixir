defmodule ADK.Sessions.Migration.DatabaseSchemaTest do
  ...
  test "new db uses latest schema" do
    ...
    [_, template] =
      Regex.run(~r/defp migration_template do\s+"""\s*(.*?)\s*"""\s+end/s, task_code)

    # Replace EEx binding
    template =
      String.replace(template, "<%= inspect @module %>", "ADK.Migrations.CreateAdkSessionsTest")

    Code.eval_string(template)

    Ecto.Migrator.up(TestRepo, 20_260_101_000_000, ADK.Migrations.CreateAdkSessionsTest)

    # Verify metadata table
    result =
      Ecto.Adapters.SQL.query!(
        TestRepo,
        "SELECT name FROM sqlite_master WHERE type='table' AND name='adk_internal_metadata'"
      )

defmodule ADK.StreamingAudioStorageParityTest do
  ...
  test "test_audio_caching_direct", %{ctx: ctx, session_id: session_id} do
    ...
    assert :ok == artifact_mod.save(ctx.app_name, ctx.user_id, session_id, "audio_1.pcm", artifact, artifact_opts) |> elem(0)
    ...
    {:ok, loaded_artifact} = artifact_mod.load(ctx.app_name, ctx.user_id, session_id, audio_key, artifact_opts)
  ...
  test "test_transcription_handling", %{ctx: ctx, session_pid: session_pid} do
    ...
    transcription_events = Enum.filter(events_after, fn event -> 

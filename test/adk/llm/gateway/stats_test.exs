defmodule ADK.LLM.Gateway.StatsTest do
  ...
  test "record_request updates counters" do
    Stats.record_request(:test_backend, 0, %{latency_ms: 100, tokens_in: 10, tokens_out: 20, status: :ok})
  ...
  test "get_stats returns aggregates" do
    for _ <- 1..5 do
      Stats.record_request(:agg_test, 0, %{latency_ms: 50, tokens_in: 5, tokens_out: 10, status: :ok})
    end
  ...
  test "p50/p95 latency calculation" do
    for i <- 1..100 do
      Stats.record_request(:latency_test, 0, %{latency_ms: i, tokens_in: 0, tokens_out: 0, status: :ok})
    end
  ...
  test "error_rate calculation" do
    for _ <- 1..80 do
      Stats.record_request(:err_test, 0, %{latency_ms: 10, tokens_in: 0, tokens_out: 0, status: :ok})
    end
    for _ <- 1..20 do
      Stats.record_request(:err_test, 0, %{latency_ms: 10, tokens_in: 0, tokens_out: 0, status: :error})
    end
  ...
  test "reset clears stats" do
    Stats.record_request(:reset_test, 0, %{latency_ms: 10, tokens_in: 1, tokens_out: 1, status: :ok})
    Stats.reset()
